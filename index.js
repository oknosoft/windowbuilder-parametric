  ...
  class Pricing {
    ...
    sync_local(pouch, step = 0) {
      return pouch.remote.doc.get(`_local/price_${step}`).then(remote => {
        return pouch.local.doc.get(`_local/price_${step}`).then(local => local).catch(() => {}).then(local => {
      ...
      this.recalc = function (scheme, attr) {
        ...
        if (attr.save) {
          ...
          saver.then(() => {
            ...
            // console.profileEnd();
          }).then(() => scheme._scope && setTimeout(() => ox.calc_order._modified && ox.calc_order.save(), 1000)).catch(ox => {
...
* @extends CatObj
* @constructor 
*/class CatElm_visualization extends CatObj{get svg_path(){return this._getter('svg_path');}set svg_path(v){this._setter('svg_path',v);}get note(){return this._getter('note');}set note(v){this._setter('note',v);}get attributes(){return this._getter('attributes');}set attributes(v){this._setter('attributes',v);}get rotate(){return this._getter('rotate');}set rotate(v){this._setter('rotate',v);}get offset(){return this._getter('offset');}set offset(v){this._setter('offset',v);}get side(){return this._getter('side');}set side(v){this._setter('side',v);}get elm_side(){return this._getter('elm_side');}set elm_side(v){this._setter('elm_side',v);}get cx(){return this._getter('cx');}set cx(v){this._setter('cx',v);}get cy(){return this._getter('cy');}set cy(v){this._setter('cy',v);}get angle_hor(){return this._getter('angle_hor');}set angle_hor(v){this._setter('angle_hor',v);}get priority(){return this._getter('priority');}set priority(v){this._setter('priority',v);}get predefined_name(){return this._getter('predefined_name');}set predefined_name(v){this._setter('predefined_name',v);}}$p.CatElm_visualization=CatElm_visualization;$p.cat.create('elm_visualization');/**
