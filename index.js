...
const MetaEngine = __webpack_require__(10).plugin(__webpack_require__(11));
debug('required');
...
__webpack_require__(12)($p);

// модификаторы data-объектов в старом формате
const modifiers = __webpack_require__(13);
...
(async () => {
  ...
  const config_init = __webpack_require__(14);

  // функция инициализации структуры метаданных
  const meta_init = __webpack_require__(16);
...
app.use(__webpack_require__(19).middleware());
...
var _cch_properties = __webpack_require__(8);
...
module.exports = function ($p) {
  ...
  $p.DocCalc_order = class DocCalc_order extends $p.DocCalc_order {
    ...
    before_save() {
      ...
      const res = this._manager.pouch_db.query('linked', { startkey: [this.ref, 'cat.characteristics'], endkey: [this.ref, 'cat.characteristics\u0fff'] }).then(({ rows }) => {
        const deleted = [];
      ...
      if (this._data.before_save_sync) {
        return res.then(() => rows_saver).then(() => this);
      }
    }
    ...
    _mgr.form_obj = function (pwnd, attr, handlers) {
      ...
      attr.draw_pg_header = (o, wnd) => {
        ...
        wnd.elmnts.pg_left = wnd.elmnts.cell_left.attachHeadFields({
          ...
          oxml: {
            ' ': [{ id: 'number_doc', path: 'o.number_doc', synonym: 'Номер', type: 'ro' }, { id: 'date', path: 'o.date', synonym: 'Дата', type: 'ro', txt: moment(o.date).format(moment._masks.date_time) }, 'number_internal'],
            'Контактная информация': ['partner', { id: 'client_of_dealer', path: 'o.client_of_dealer', synonym: 'Клиент дилера', type: 'client' }, 'phone', { id: 'shipping_address', path: 'o.shipping_address', synonym: 'Адрес доставки', type: 'addr' }],
            'Дополнительные реквизиты': ['obj_delivery_state', 'category', { id: 'manager', path: 'o.manager', synonym: 'Автор', type: 'ro' }, 'leading_manager']
          }
        });
            ...
            if (row) {
              ...
              if (row.characteristic.empty() || calc_order.empty() || owner.is_procedure || owner.is_accessory) {
                ...
                o.create_product_row({ grid: wnd.elmnts.grids.production, create: true }).then(({ characteristic }) => {
                  ...
                  characteristic._mixin(row.characteristic._obj, null, 'ref,name,calc_order,product,leading_product,leading_elm,origin,note,partner'.split(','), true);
  ...
  (_mgr => {
    ...
    _mgr.clone = async function (src) {
      ...
      // создаём заказ
      const _src$_obj = src._obj,
            { organization, partner, contract } = _src$_obj,
            others = _objectWithoutProperties(_src$_obj, ['organization', 'partner', 'contract']);
      ...
      src.production.forEach(row => {
        ...
        if (row.characteristic.calc_order === src) {
          ...
          if (row.characteristic._attachments) {
            aatt.push(db.getAttachment(`cat.characteristics|${row.characteristic.ref}`, 'svg').then(att => cx._obj._attachments = { svg: { content_type: 'image/svg+xml', data: att } }).catch(err => null));
          }
        }
  ...
  $p.doc.calc_order.__define({
    ...
    rep_invoice_execution: {
      value(rep) {
        ...
        return $p.adapters.pouch.remote.doc.query("server/invoice_execution", query_options).then(function (data) {
    ...
    rep_planing: {
      value(rep, attr) {
        ...
        return $p.adapters.pouch.remote.doc.query('server/planning', query_options).then(function (data) {
  ...
  class Pricing {
    ...
    sync_local(pouch, step = 0) {
      return pouch.remote.doc.get(`_local/price_${step}`).then(remote => {
        return pouch.local.doc.get(`_local/price_${step}`).catch(() => ({})).then(local => {
        ...
        if (step !== 0) {
          pouch.local.doc.get(`_local/price_${step}`).then(local => pouch.local.doc.remove(local)).catch(() => null);
          return true;
        }
      });
    }
    ...
    by_local(step = 0) {
      ...
      const pre = step === 0 && pouch.local.doc.adapter !== 'http' && $p.adapters.pouch.authorized ? pouch.remote.doc.info().then(() => this.sync_local(pouch)).catch(err => null) : Promise.resolve();
      ...
      this.recalc = function (scheme, attr) {
        ...
        if (attr.save) {
          ...
          ox.save().then(() => {
            ...
            // console.profileEnd();
          }).then(() => (scheme._scope || attr.close) && setTimeout(() => ox.calc_order._modified && ox.calc_order.save(), 1000)).catch(ox => {
  ...
  if (typeof window !== 'undefined') {
    window.ProductsBuilding = ProductsBuilding;
  }
...
try{formula.execute();}catch(err){}}});}// переопределяем load_array - не грузим неактивные формулы
...
* @extends CatObj
* @constructor 
*/class CatElm_visualization extends CatObj{get svg_path(){return this._getter('svg_path');}set svg_path(v){this._setter('svg_path',v);}get note(){return this._getter('note');}set note(v){this._setter('note',v);}get attributes(){return this._getter('attributes');}set attributes(v){this._setter('attributes',v);}get rotate(){return this._getter('rotate');}set rotate(v){this._setter('rotate',v);}get offset(){return this._getter('offset');}set offset(v){this._setter('offset',v);}get side(){return this._getter('side');}set side(v){this._setter('side',v);}get elm_side(){return this._getter('elm_side');}set elm_side(v){this._setter('elm_side',v);}get cx(){return this._getter('cx');}set cx(v){this._setter('cx',v);}get cy(){return this._getter('cy');}set cy(v){this._setter('cy',v);}get angle_hor(){return this._getter('angle_hor');}set angle_hor(v){this._setter('angle_hor',v);}get priority(){return this._getter('priority');}set priority(v){this._setter('priority',v);}get mode(){return this._getter('mode');}set mode(v){this._setter('mode',v);}get predefined_name(){return this._getter('predefined_name');}set predefined_name(v){this._setter('predefined_name',v);}}$p.CatElm_visualization=CatElm_visualization;$p.cat.create('elm_visualization');/**
...
router.loadMethods().get('/:class/:ref', __webpack_require__(2)).post('/:class/:ref', __webpack_require__(21));
