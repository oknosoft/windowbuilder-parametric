  ...
  $p.DocCalc_order = class DocCalc_order extends $p.DocCalc_order {
    ...
    print_data() {
      ...
      const res = {
        ...
        СотрудникФИО: manager.individual_person.Фамилия + (manager.individual_person.Имя ? ' ' + manager.individual_person.Имя[1].toUpperCase() + '.' : '') + (manager.individual_person.Отчество ? ' ' + manager.individual_person.Отчество[1].toUpperCase() + '.' : ''),
      ...
      this.production.forEach(row => {
        ...
        if (!row.characteristic.empty() && !row.nom.is_procedure && !row.nom.is_service && !row.nom.is_accessory) {
          ...
          get_imgs.push($p.cat.characteristics.get_attachment(row.characteristic.ref, 'svg').then(blob => $p.utils.blob_as_text(blob)).then(svg_text => {
    ...
    fill_plan() {
      ...
      .then(() => {
        ...
        headers.append('Authorization', 'Basic ' + btoa(unescape(encodeURIComponent($p.wsql.get_user_param('user_name') + ':' + $p.aes.Ctr.decrypt($p.wsql.get_user_param('user_pwd'))))));
    ...
    load_production(forse) {
      ...
      return mgr.adapter.load_array(mgr, prod).then(() => {
        prod.length = 0;
        this.production.forEach(row => {
          const { nom, characteristic } = row;
