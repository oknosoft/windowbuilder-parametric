...
В тело ответа помещается стандартный JSON заказа без детальной информации о продукциях

Планируется дополнить метод `recalc`, возможностью переопределить реквизиты шапки заказа (Пользователь, Организация, Подразделение, Учёт НДС и т.д.)
