...
````javascript
// Замена организации заказа. В этом случае, стандартный отбработчик заменит и договор для пары контрагент-организация
{
    "action": "recalc",
    "organization": ""
}
...
// Замена строк продукции (табчасть заменяется полностью, реквизиты строк `price`, `price_internal`, `marginality`,
// `amount`, `amount_internal`, `vat_amount`, `first_cost`, `margin` - можно не указывать,
